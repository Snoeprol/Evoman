...
def mutate(individual):
    ...
    individual.stdevs *= np.exp(tau_2 * np.random.normal(0,1, len(individual.stdevs)) + tau * np.random.normal(0,1, len(individual.stdevs)))
    '''
    ...
    covariance = generate_covariance_matrix(individual.stdevs, individual.alphas)
